...
- The VIF of a feature is: $VIF = \frac{1}{1 - R^2}$, where $R^2$ is the coefficient of determination of a regression of that feature on all the other features.
...
- Lasso is a linear regression model with L1 regularisation, which can shrink some coefficients to zero. Features with zero coeffcients are effectively removed.
- L1 norm of a vector: $$\|\mathbf{x}\|_1 = \sum_{i=1}^{n} |x_i|$$
- α is hyperparameter that controls regularisation. Higher α means more regularisation, which leads to more discarded features.
...
comparison_data = {
  "": ["Formula", "To minimise"],
  "OLS": ["y = Xβ", "$\min_{\beta} \left\{ \frac{1}{2n} \sum_{i=1}^{n} (y_i - \mathbf{x}_i^T \beta)^2 \right\}$"],
  "Lasso": ["y = Xβ", "$\min_{\beta} \left\{ \frac{1}{2n} \sum_{i=1}^{n} (y_i - \mathbf{x}_i^T \beta)^2 + \alpha \|\beta\|_1 \right\}$"]
}
...
| **Interpretation** | VIF > 5-10 indicates multicollinearity | α controls sparsity |
...
- MI is **model-dependent**.
- MI can capture non-linear relationships, which is more general than Pearson correlation.
...
<div style="text-align:center;">
  <img src="images/permutation_importance_illustration.png" style="max-width:70%;">
</div>
...
<div style="text-align:center;">
  <img src="images/RFECV_illustration.png" style="max-width:70%;">
</div>
