...
Most of the working is same as for the PyTorch version, except that we will be using an adjacency list representation instead of adjacency matrix. The advantage of using adjacency list is that all the edges can be stored in $O(|E|)$ where $|E|$ is the number of edges, whereas in adjacency matrix it is $O(|V|^2)$ where $|V|$ is number of vertices. So if the number of edges is much less than the number of vertices squared then adjacency list can save a lot of memory and time.
...
![Zachary Karate Club](images/karate_club.png)
...
![Simple Graph](images/example_graph_1.png)
...
![One Layer Aggregation](images/aggregation_1.png)
...
![Animation Aggregation Layer 1](images/animation_2_bw.gif)

So after one iteration, the values are as follows:

![Second Layer Aggregation](images/aggregation_2.png)

Repeating the same procedure on this new graph, we get (try verifying yourself):

![Third Layer Aggregation](images/aggregation_3.png)
...
![Sampling 2](images/showing_2.png)
...
![GraphSAGE Algorithm](images/graphsage_algorithm.png)
...
![Sharing Parameters](images/sharing_param.png)
...
![Protein Interaction](images/protein.png)
